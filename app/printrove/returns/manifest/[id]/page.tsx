...
export default function ReturnManifestDetailPage() {
  ...
  const handleScanSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const scanResult = await apiClient.put(
        `/return-manifest/${manifestId}/scan`,
        {
      ...
      if (scanData.success && scanData.data.order) {
        ...
        const addResult = await apiClient.post(
          `/return-manifest/${manifestId}/order`,
          {
  ...
  const handleRemoveOrder = async (orderId: string) => {
    ...
    try {
      const result = await apiClient.delete(
        `/return-manifest/${manifestId}/order/${orderId}`
      );
  ...
  // Save product inventory types
  const handleSaveProducts = async (orderId: string) => {
    if (!currentManifest || !inventoryTypes[orderId]) return;

    setIsSavingProducts(true);

    try {
      // Find order in manifest
      const order = currentManifest.orders.find((o) => o.orderId === orderId);

      if (!order) {
        throw new Error("Order not found in manifest");
      }
      ...
      if (
        ...
        localScannedOrders[orderId]
      ) {
        orderProducts = localScannedOrders[orderId].products || [];
      }
      ...
      // Prepare products data for API - use "Good_Inventory" and "Bad_Inventory" instead of "GOOD" and "BAD"
      const productsForAPI = orderProducts.map((product) => {
        ...
        const rawType =
          inventoryTypes[order.orderId]?.[productId] || "Good_Inventory";

        return {
          productId,
          quantity: product.qty || 0,
          inventoryType: rawType, // Send exactly as is - should be "Good_Inventory" or "Bad_Inventory"
        };
      });

      console.log("Sending products to API:", productsForAPI);
      ...
      if (success) {
        showToast(`Saved inventory types for order ${orderId}`);
      ...
      let message = "Failed to save product inventory types";
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      const success = await completeReturnManifest(manifestId);

      if (success) {
        showToast("Return manifest marked as complete", false);

        // Navigate back to list after short delay
        setTimeout(() => {
          router.push("/printrove/returns/manifest");
        }, 1500);
      }
    } catch (error: any) {
  ...
  const canMarkComplete = () => {
    ...
    if (!currentManifest || currentManifest.status === "COMPLETED")
      return false;
    ...
    return (
      <EmptyState
        ...
        action={{
          ...
          onAction: () => router.push("/printrove/returns/manifest"),
        }}
      >
        <p>
          The return manifest you're looking for doesn't exist or may have been
          deleted.
        </p>
      </EmptyState>
    );
          ...
          <Card>
            <div className="p-5">
              {currentManifest.orders?.length ? (
                      ...
                      <div>
                        ...
                        <DataTable
                          ...
                          rows={(
                            ...
                            ]; // Removed the 4th column (save button)
                          })}
                        />
                ...
                <Banner tone="info">
                  <p>
                    No orders have been added to this return manifest yet. Scan
